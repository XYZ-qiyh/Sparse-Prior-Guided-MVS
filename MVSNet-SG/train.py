...
parser.add_argument('--k', type=float, default=10, help='the amplitude of Gaussian Modulation Function')
parser.add_argument('--c', type=float, default=2, help='the bandwidth of Gaussian Modulation Function')
...
# model, optimizer
model = MVSNet(refine=False, k=args.k, c=args.c)
...
def test():
    ...
    '''print to txt file'''
    with open("./depthmap_valid2.txt", 'a') as f:
        # print("testlist: {}".format(args.testlist), file=f)
        print("\nuse_guided: {}".format(args.use_guided), file=f)
        if args.use_guided:
            print("k={}, c={}".format(int(args.k), args.c), file=f)
        print("final: {}\n".format(avg_test_scalars.mean()), file=f)
...
def test_sample(sample, detailed_summary=True):
    ...
    if not args.use_guided:
        ...
        outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"],
                        use_guided=args.use_guided, depth_gt=depth_gt)
